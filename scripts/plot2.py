...
#filenames
l1 = ['22a','23a','23c','24a','24c','25c','26a','27b','27c','28a']
l2 = ['22b','23b','23d','24b','24d','25a','25d','26b','27d','27e','28c','28d']
avg = 10
stp= 0.2
xx = []
yy = []
...
for i in range(0,np.size(l2)):
   ...
   # if l2[i] == '28a':
    ...
    for j in range(0,np.size(x)):
        ...
        ys.append(ynew)
    #plt.plot(xs,ys,'o')
    if l2[i] in ('25d','26b','27e','27d','28d','28c'):
...
plt.plot(x1s,y1s)
np.savetxt('../scans/2a.dat',np.column_stack((x1s,y1s)))
...
x1,y1 = np.loadtxt('../scans/nx022b_165.dat',usecols=(1,2),unpack=True)
x1 = x1[:-1]
y1 = y1[:-1]
#x1 = x1[20:]
#y1 = y1[20:]
...
plt.plot(x1s,y1s,'--')
np.savetxt('../scans/2b.dat',np.column_stack((x1s,y1s)))
